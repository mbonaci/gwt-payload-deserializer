...
public abstract class AbstractSerializationStreamReader extends AbstractSerializationStream implements SerializationStreamReader {
  ...
  */
  ...
  public final Object readObject() throws SerializationException {
	  ...
	  //    System.out.println("AbstractSerializationStreamReader.readObject token = " + token);

	  if (token < 0) {
		  // Negative means a previous object
		  // Transform negative 1-based to 0-based.
		  Object tmpObj = seenArray.get(-(token + 1));
		  PayloadDeserializer.appendToFile("\t" + tmpObj.getClass().getSimpleName());
		  PayloadDeserializer.appendToFile("\t" + tmpObj.toString());
		  return tmpObj;
	  }
	  ...
	  //    System.out.println("AbstractSerializationStreamReader.readObject typeSignature = " + typeSignature);
	  Object tmpObj = deserialize(typeSignature);
