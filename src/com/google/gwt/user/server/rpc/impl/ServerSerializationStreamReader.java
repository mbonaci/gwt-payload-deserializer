 ...
 */
public class ServerSerializationStreamReader extends AbstractSerializationStreamReader {
	...
	private static class BoundedList<T> extends LinkedList<T> {
		 ...
		 * 
		 */
		private static final long serialVersionUID = 947808434031682166L;
	 ...
	 * Class type and a field name corresponding to a field of type T
	 */
	...
	private boolean skipPrintingValue;
	...
	public Object deserializeValue(Class<?> type) throws SerializationException {
		...
		if (valueReader != null) {
			...
			tmpObj = ValueReader.OBJECT.readValue(this);
	...
	public void prepareToRead(String encodedTokens) throws SerializationException {
		...
		if (getVersion() < SERIALIZATION_STREAM_MIN_VERSION || getVersion() > SERIALIZATION_STREAM_VERSION) {
		...
		if (clientFieldNames != null) {
			...
			try {
				...
				if (encodedData != null) {
					...
					System.out.println("Base64 encoded count = " + count);
...
//			PayloadDeserializer.appendToFile("###################################################\n\n");
			...
			if ((setters != null) && ((setter = setters.get(fieldName)) != null)) {
				...
				try{
					...
					PayloadDeserializer.appendToFile("\t%%%%%%%%%%   NOT ASSIGNABLE   %%%%%%%%%%%");
...
//				}
	...
	private Object instantiate(Class<?> customSerializer, Class<?> instanceClass)
		...
		if (instanceClass.isArray()) {
			int length = readInt();
			PayloadDeserializer.appendToFile("\tarray size");
